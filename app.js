...
const fetchUrl = async(url) => {
    ...
    ulPopup.setAttribute("id", "ul");
    ulPopup.innerHTML = `
        <li>Long URL: <a href=${data.result.original_link} target="_blank">${data.result.original_link}</a></li>
...
containerPopUp.addEventListener("click", (e) => {
    if(e.target.classList.contains('popup-container')){
        const ulPopup = document.getElementById('ul');
        ulPopup.remove();
